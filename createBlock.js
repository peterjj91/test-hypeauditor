...
if (blockName) {
  ...
  mkdirp(dirPath, (err) => {                                           // создаем
    ...
    if (err) {
      console.error(`[Agima] Отмена операции: ${err}`);
    }

    // Нет ошибки, поехали!
    else {
      console.log(`[Agima] Создание папки ${dirPath} (если отсутствует)`);
      ...
      extensions.forEach((extention) => {
        ...
        if (extention === 'scss') {
          ...
          if (!hasThisBlock) {
            ...
            fileCreateMsg = '[Agima] Подключение блока добавлено в projectConfig.json';
          }
        }
        ...
        // Если нужна подпапка для картинок
          ...
          if (fileExist(imgFolder) === false) {
            mkdirp(imgFolder, (err) => {
              ...
              else console.log(`[Agima] Создание папки: ${imgFolder} (если отсутствует)`);
            });
          } else {
            console.log(`[Agima] Папка ${imgFolder} НЕ создана (уже существует) `);
          }
        }
        ...
        if (fileExist(filePath) === false && extention !== 'img') {
          fs.writeFile(filePath, fileContent, (err) => {
            if (err) {
              return console.log(`[Agima] Файл НЕ создан: ${err}`);
            }
            console.log(`[Agima] Файл создан: ${filePath}`);
            if (fileCreateMsg) {
              console.warn(fileCreateMsg);
            }
          });
        } else if (extention !== 'img') {
          console.log(`[Agima] Файл НЕ создан: ${filePath} (уже существует)`);
        }
      });
    }
  });
} else {
  console.log('[Agima] Отмена операции: не указан блок');
}
